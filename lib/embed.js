...
(function() {
	...
	function getProductDetails( product, callback ){
		ajax( url + "product/info/"+product, function( response ){
			// stop if there is no real product
			callback( response );
		});
	};
	...
	function createButton( data, el ){
		...
		if( long_text || short_text || prompt ){
			var span = document.createElement("span");
			var default_prompt = ( long_text ) ? "Subscribe with" : "Subscribe";
...
// check if there is a subscriber authenticated
window.onscribe.isAuth = function(){
